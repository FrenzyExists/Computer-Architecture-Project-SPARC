...
# while true; do mpv $M > /dev/null 2>&1; done &
...
select_options() {
    ...
    while true; do
        ...
        if [ "$key" == "" ]; then
            ...
            if [ -n "$selected_func" ]; then
                if [ "$selected_func" == "$_exit_func" ]; then
                    break
                else
                    
                    function_stack+=("select_options '${_options[*]}' '${_functions[*]}' '$_exit_func'")
                    "$selected_func"
                fi
            fi
            sleep 3
    ...
    if [ ${#function_stack[@]} -gt 0 ]; then
        local prev_func="${function_stack[-1]}"
...
# Define the options and corresponding functions for the main menu
options=("Main Option 1" "Main Option 2" "Submenu" "Exit")
functions=("main_function1" "main_function2" "submenu" "exit_program")

# Set initial cursor position
cursor_pos=0

# Call the select_options function
# select_options "options" "functions" "exit_program"

# Function 1 for the submenu
# Submenu function
submenu() {
    # Define the options and corresponding functions for the submenu
    local submenu_options=("Submenu Option 1" "Submenu Option 2" "Go Back")
    local submenu_functions=("submenu_function1" "submenu_function2" "go_back")

    # Set initial cursor position
    cursor_pos=0
    sleep 3
    # Call the select_options function for the submenu
    select_options "submenu_options" "submenu_functions" "go_back"
}

# Function 1 for the submenu
submenu_function1() {
    echo "Executing Submenu Function 1"
    sleep 2
}

# Function 2 for the submenu
submenu_function2() {
    echo "Executing Submenu Function 2"
    sleep 2
}
...
main_menu() {
    # Define the options and corresponding functions for the main menu
    local main_options=("Main Option 1" "Main Option 2" "Submenu" "Exit")
    local main_functions=("main_function1" "main_function2" "submenu" "exit_program")

    # Set initial cursor position
    cursor_pos=0

    # Call the select_options function for the main menu
    select_options "main_options" "main_functions" "go_back"
}

# Function 1 for the main menu
main_function1() {
    echo "Executing Main Function 1"
    sleep 2
}

# Function 2 for the main menu
main_function2() {
    echo "Executing Main Function 2"
    sleep 2
}
